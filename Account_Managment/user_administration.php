...
<head>
    ...
    <?php include '../headerNew.php'; include $_SERVER['DOCUMENT_ROOT'].'/sidebar.php'; ?>
    <style>
    body {
...
<body>
    <div class="main-content">
        <div class="glass-panel shadow">
            ...
            <div class="mb-4 position-relative" style="margin-bottom:36px;">
                <label for="user-searchbox" class="form-label" style="font-size:1.18rem;font-weight:700;">Find
                    User</label>
                <input id="user-searchbox" class="form-control"
                    style="background:#23243a;color:#fff;padding:12px 2px;border-radius:10px;border:none;"
                    autocomplete="off" placeholder="Type username or email..." onfocus="showUserList()"
    ...
    <div class="toast-container p-3 position-fixed">
        <?php if ($type_message): ?>
        <div id="useradm-toast" class="toast show text-bg-success">
            <div class="d-flex">
                <div class="toast-body"><?= $type_message ?></div><button type="button"
                    class="btn-close btn-close-white m-auto" data-bs-dismiss="toast"></button>
            </div>
        </div>
        <?php elseif ($del_message): ?>
        <div id="useradm-toast" class="toast show text-bg-danger">
            <div class="d-flex">
                <div class="toast-body"><?= $del_message ?></div><button type="button"
                    class="btn-close btn-close-white m-auto" data-bs-dismiss="toast"></button>
            </div>
        </div>
        <?php endif; ?>
...
?>
    ...
    function filterUserList() {
        ...
        let filtered = allUsers.filter(u => u.toLowerCase().includes(val)).sort((a, b) => a.localeCompare(b, "en", {
            sensitivity: "base"
        }));
        acList.innerHTML = "";
        if (!val) {
